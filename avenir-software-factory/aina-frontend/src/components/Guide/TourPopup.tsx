// src/components/Tour/TourPopup.tsx
...
const TourPopup: React.FC<TourPopupProps> = ({
  ...
  useEffect(() => {
    ...
    const calculatePosition = () => {
      ...
      const positions = {
        top: {
          top: targetRect.top - popupHeight - arrowSize - margin - offset,
  ...
  const Arrow = () => {
    ...
    const arrowClasses = {
      top: 'absolute -top-3 left-1/2 transform -translate-x-1/3 w-6 h-6',
      bottom: 'absolute -bottom-3 left-1/2 transform -translate-x-1/3 w-6 h-6',
      left: 'absolute -left-3 top-1/2 transform -translate-y-1/3 w-6 h-6',
      right: 'absolute -right-3 top-1/2 transform -translate-y-1/3 w-6 h-6'
    };
  ...
  return (
    <>
      {!isScrolling && ( // Ne pas afficher pendant le scroll
        (<div
          style={{
            position: 'fixed',
            top: `${position.top}px`,
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onNext}
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-sm font-medium hover:from-blue-600 hover:to-indigo-700 transition-all duration-200"
              >
