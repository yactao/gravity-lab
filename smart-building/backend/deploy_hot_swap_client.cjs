...
const frontendFile = '../frontend/src/app/clients/[id]/page.tsx';
...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        console.log('📤 Upload du fichier modifié...');

        const fStream = sftp.createWriteStream('/opt/gravity-lab/smart-building/frontend/src/app/clients/[id]/page.tsx');
        fStream.write(frontendContent);
        fStream.end();

        fStream.on('close', () => {
            console.log('✅ page.tsx (Frontend Client) uploadé.');
